...
then
	...
	wget -O ~/esp_32.bin https://micropython.org/resources/firmware/esp32-20220117-v1.18.bin
fi
...
then
	for sysdevpath in $(find /sys/bus/usb/devices/usb*/ -name dev); do
		...
		#	usb_path=$("$device" | awk -F' -' '{print $1}')
			...
			esptool.py --port $device_port --baud 115200 write_flash --flash_size=detect 0 ~/esp_32.bin
